...
Content-based filtering methods classify items based on the user's input on the content (e.g., like or dislike an item) to generate recommendations. On the other hand, collaborative filtering algorithms create recommendations based on the user-user relationship and their interactions with the same items.


This work focuses on collaborative filtering recommender systems, of which GraphJet \cite{sharmaGraphJetRealtimeContent2016} (explained in section \ref{subsec:GraphJet-Recommendation-Engine}) is an example. The approach uses a content-based partitioning method to distribute this system on multiple instances and increase the recommendation quality.
...
\emph{Collaborative filtering (CF)} term, which was first introduced in \cite{goldbergUsingCollaborativeFiltering1992} means that the users in a system collaborate to perform filtering by interacting with documents and record their reactions.
...
Evaluating modern information retrieval systems with recall is obsolete. These systems retrieve thousands of relevant documents that few users will be interested in reading.
...
Tables \ref{tab:apk-example} shows two ranked lists (objective, observed) of movie recommendations. If we consider all the results of the objective rankings as relevant, then the AP@5 for the observed ranking is calculated as follow:
...
When examining the ranked result list, highly relevant items (i.e., documents) are valuable to the user than less relevant ones. In other words, the higher the rank position of a relevant item, the less valuable it gets for the user. The authors of \cite{jarvelinCumulatedGainbasedEvaluation2002} define \emph{Cumulative Gain (CG)} and \emph{Discounted Cumulative Gain(DCG)} as follow: This metric measures the cumulative gain the user obtains by examining the retrieval result up to a given ranked position. 


CG also uses a so-called \emph{relevance score} for its computation. A relevance score is a number determining how relevant an item is. This score depends on the ranking and scoring algorithm of the system. For example, the relevance score can be defined as a boolean score, hit number, or self-defined rankings. In the boolean score, the relevance score is either zero or one. The hit number used in an algorithm like SALSA \cite{lempelSALSAStochasticApproach2001} is the count visit of each item. The resulting ranked list is ordered descending base on the hit number of each item. Self-defined scores are numbers determining the relevancy of each item. For example, a recommender system could only score items based on three numbers: items with a score of zero are not relevant at all, items with a score of one or two are somewhat relevant, and items with a score of three are highly relevant for the user. This work uses the idea behind the relevancy score to generate self-defined scores for the baseline results. For more details pleaser refer to section \ref{sec:evaluation-pipeline}.


\paragraph*{Cumulative Gain (CG)}
is the sum of the relevance scores (i.e. hit count) of a all items in a ranked list. Mathematically defined as follow:
...
CG does not consider the ranking of the result list. For example the two relave score list: rel\_score\_a = [0, 1, 4, 0, 3] and rel\_score\_b = [4, 3, 1, 0, 0]. The CG of both of these lists is 8, but it is obvious that the \emph{rel\_score\_b} has a better order of the relevant items. \emph{Discounted Cumulative Gain(DCG)} overcomes this problem.
...
produces the maximum possible DCG through position \emph{p} by soritig all items in the list according through their relative score. NDCG is computet as follow:
...
Where \emph{$REL_p$} indicates the ordered relevant documents by their relevance up to position \emph{p}. NDCG result varies between the value zero to one. If NDCG is equal to one, then IDCG is the same as DCG and representing a perfect score. The NDCG values can be averaged over all the requested queries to obtain the averaged performance of the recommender system.


Although NDCG is a good metric for comparing weighted ranked list, but it fails to determine the ranking quality of disjoint lists. For example consider two result list with the score of 3,2,1 and 3,2,1,0 respecitvley, both of these these lists will have the same score. Another issue with disjoint ranked list arises when documents do not accure in both lists. Consider the two lists with the score of 3,2,1 and 3,2,1,1 respecitvley, both of these lists are denoted as perfectly equal.
...
When doing A/B testing of recommendation systems, the experiments fall into three main categories: First, the recommendation algorithm (i.e., new rankings, data fusion, recommendation parameters). The second would be the distribution of a system (i.e., how many instances one system is running)—lastly, showing recommendations only to a specific group of users under test. We can create different test scenarios based on these three categories where system A is assessed against system B. By running each scenario, the feedback data is gathered and statically analyzed to determine if the newly added changes improved the recommendation quality or not.
...
Studies show that data fusion methods in IR systems are complex and depend highly on the system architecture and algorithm used in the system \cite{frankhsuComparingRankScore2005}. In IR systems, specialy search engines, the incoming query get processed in a so-called query \emph{formulation} step. In this step the incomming query get tokenized, normalized,  and the stop words are removed. This step transforms the query into a digestable representation for the search engine. Afterward, a \emph{schema} processes the processed query and generates the results. 


Single instance recommender systems take a query as their imput and produce a single ranked list of recommendations for a user. In recommendation systems the incomming query can only be a simple user-ID and does not need to go through the formulation step. Figure \ref{fig:data-fusion-simple} represents this behaviour.


\begin{figure}[!h]
	\centering
	\includegraphics[width=0.75\textwidth]{images/simple-formulation-schema}
...
Multiple formulations can use different normalization or tokanization algorithm to process the incomming queries. The end results of each formulation varies even with the same schema.


This work uses an SFMS architecture for the distribution. In this architecture, the query is sent to a single formulation point and from there to multiple schemes (i.e., workers). Each scheme runs the recommendation algorithm and produces a ranked list. In the last step, these ranked lists are then integrated into a single ranked list. The section \ref{sec:data-fusion-approaches} explains the data fusion approaches used in this work. For more detail about the distributed architecture pleaser refer to section \ref{sec:system-architecture}.
